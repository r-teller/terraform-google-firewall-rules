...
This module supports both IPv4 and IPv6 firewall rules. The IP version is automatically inferred from existing CIDRs in the rule, with the `implicit_ip_version` variable serving as a fallback.

**Inference Logic** (applied in order):
1. **If CIDRs are specified**: Uses the specified CIDRs directly
2. **If only tags/service-accounts**: Infers IP version from the opposite field's CIDRs
3. **No CIDRs to infer from**: Falls back to `implicit_ip_version` (default: IPV4)

**`implicit_ip_version` Variable**:
- **IPV4 (default)**: Uses `0.0.0.0/0` when no CIDRs are present to infer from
- **IPV6**: Uses `::/0` when no CIDRs are present to infer from
...
**Example 1: Inferred from opposite field**
```json
// Tag in sources + IPv6 CIDR in targets → infers IPv6 for sources
{
  "sources": ["web-server"],      // tag (no CIDR)
  "targets": ["fd00::/8"],        // IPv6 CIDR
  "rules": [...]
}
// Result: sources = ["::/0"] (inferred), targets = ["fd00::/8"] ✅
```

**Example 2: Inferred from opposite field**
```json
// IPv4 CIDR in sources + tag in targets → infers IPv4 for targets
{
  "sources": ["10.0.0.0/8"],      // IPv4 CIDR
  "targets": ["backend"],         // tag (no CIDR)
  "rules": [...]
}
// Result: sources = ["10.0.0.0/8"], targets = ["0.0.0.0/0"] (inferred) ✅
```

**Example 3: Falls back to implicit_ip_version**
```json
// Only tags, no CIDRs → uses implicit_ip_version (IPV4 default)
{
  "sources": ["web-server"],      // tag (no CIDR)
  "targets": ["backend"],         // tag (no CIDR)
  "rules": [...]
}
// Result: sources = ["0.0.0.0/0"], targets = ["0.0.0.0/0"] (from variable)
// To get IPv6: set implicit_ip_version = "IPV6"
```

**Example 4: Empty → no implicit addresses**
```json
...
**Example 5: INVALID - Mixed IP versions**
```json
// Mixing IPv4 and IPv6 CIDRs → Terraform validation error
{
...
module "firewall_rules" {
  ...
  # Optional fallback for IP version when no CIDRs are present to infer from
  # The module always tries to infer from existing CIDRs first
  # Only used when sources/targets have only tags/service-accounts (no CIDRs)
  # Options: "IPV4" (default), "IPV6"
  # Note: GCP firewall rules are single-stack only
  implicit_ip_version = "IPV4"
...
| implicit_ip_version        | Fallback IP version when no CIDRs present to infer from. Module always infers from existing CIDRs first. Options: IPV4, IPV6. | `string` | `IPV4`       |    no    |
