locals {
    ...
    rule_direction = upper(var.firewall_rule.direction)
    ...
    source_ranges = [ for x in var.firewall_rule.sources: x if can(cidrnetmask(x)) ]
    ...
    target_ranges = [ for x in var.firewall_rule.targets: x if can(cidrnetmask(x)) ]
}
...
resource "google_compute_firewall" "firewall_rule" {
    ...
    dynamic "log_config" {
        for_each = can(var.firewall_rule.log_config) ? [1] : []
        content {
